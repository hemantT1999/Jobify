...
const AppProvider = ({ children }) => {
  ...
  // ----------Dispatching Actions------------- //
  const displayAlert = (text = "Something went wrong", type = "danger") => {
  ...
  const applyForJob = async (jobId) => {
    try {
      const { data } = await authFetch.post(`/jobs/apply/${jobId}`);

      dispatch({
        type: APPLY_JOB_SUCCESS,
        payload: {
          jobId,
          applicationDate: data.application.appliedDate,
        },
      });

      // Refresh applied jobs to get the updated list with dates
      getAppliedJobs();

      displayAlert("Successfully applied for job", "success");
    } catch (error) {
  ...
  // Fetch applied jobs on app load if user is a job seeker
  useEffect(() => {
    if (state.user && state.user.role === "job_seeker") {
      getAppliedJobs();
    }
